...
void gx_gui_create_window_and_surface(gx_saturateUI *ui) {
	...
	static TCHAR szClassName[] = TEXT("gx_saturateUIClass");
	WNDCLASS wndclass = {0};
...
void gx_gui_destroy_main_window(gx_saturateUI *ui) {
	...
	UnregisterClass(TEXT("gx_saturateUIClass"), NULL);
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	gx_saturateUI *ui = (gx_saturateUI *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
