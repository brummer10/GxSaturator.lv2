...
#define GTK_SAT_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_SAT_KNOB, GtkSatKnobPrivate))

G_DEFINE_TYPE(GtkSatKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkSatKnobPrivate GtkSatKnobPrivate;

struct _GtkSatKnobPrivate
{
	int knob_x;
	int knob_y;
	int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
	...
	GtkSatKnobPrivate *priv = GTK_SAT_KNOB_GET_PRIVATE(widget);
    if ( priv->show_value || gtk_widget_has_focus(widget)== TRUE) {
...
{
	g_assert(GTK_IS_SAT_KNOB(widget));
	GtkSatKnob *knob = GTK_SAT_KNOB(widget);
	GtkSatKnobPrivate *priv = GTK_SAT_KNOB_GET_PRIVATE(knob);
...
/****************************************************************
 ** init the GtkSatKnobClass
 */

static void gtk_knob_class_init (GtkSatKnobClass *klass)
{
...
//--------- connect the events with funktions
	...
	g_type_class_add_private(obj_class, sizeof (GtkSatKnobPrivate));
}
...
static void gtk_knob_init (GtkSatKnob *knob)
{
...
{
	GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_SAT_KNOB, NULL ));
