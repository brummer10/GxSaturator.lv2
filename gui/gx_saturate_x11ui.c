...
static void resize_event(gx_saturateUI *ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_saturateUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor * descriptor,
	...
	ui->controls[0] = { 6.0, 6.0, 1.0, 10.0, 0.1, 40, 30, 71, 71, false,"INTENSITY", KNOB, INTENSITY};
	ui->controls[1] = { 60.0, 60.0, 0.0, 100.0, 1.0, 150, 30, 71, 71, false,"SATURATE", KNOB, SATURATE};
	...
	ui->width = ui->init_width -120 + (70 * CONTROLS);
	...
	*widget = (void*)ui->win;
	...
	ui->rescale.xc = (double)ui->width/(ui->init_width-120 + (70 * CONTROLS));
...
static void knob_expose(gx_saturateUI *ui,gx_controller* knob) {
...
static void bypass_expose(gx_saturateUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(gx_saturateUI *ui, gx_controller* controller) {
...
static void _expose(gx_saturateUI *ui) {
	static const char* plug_name = "GxSaturator" ;
...
static void controller_expose(gx_saturateUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
static void send_controller_event(gx_saturateUI *ui, int controller) {
...
static void check_is_active(gx_saturateUI *ui, int i, bool set) {
...
static bool aligned(int x, int y, gx_controller *control, gx_saturateUI *ui) {
...
static bool get_active_ctl_num(gx_saturateUI *ui, int *num) {
	static bool ret;
	ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num(gx_saturateUI *ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_saturateUI *ui, int direction) {
	float value;
	int num;
...
static void switch_event(gx_saturateUI *ui, int i) {
...
static void button1_event(gx_saturateUI *ui, double* start_value) {
	int num;
	if (get_active_ctl_num(ui, &num)) {
...
static void motion_event(gx_saturateUI *ui, double start_value, int m_y) {
...
static void set_key_value(gx_saturateUI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_saturateUI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_saturateUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_saturateUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
void get_last_active_controller(gx_saturateUI *ui, bool set) {
...
static void event_handler(gx_saturateUI *ui) {
	XEvent xev;
